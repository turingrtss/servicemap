...
class LogParser:
    ...
    def _parse_line(self, line: str, source_service: str):
        ...
        try:
            ...
            # NEW: Handle gRPC calls in 'target' field (common format)
            elif 'target' in data:
                target = data['target'].split(':')[0]  # Extract service from host:port
                self.calls[service][target] += 1
                self.services.add(target)
            # NEW: Handle Redis/database commands
            elif 'command' in data and 'key' in data:
                self.calls[service]['db:redis'] += 1
